 ...
 * 16371101
 * Jordan O Donnell
 * EXTRA CREDIT - Genererating new passwords using String Pointers and Const
 */
...
void loop() 
{
  ...
  else if (sw1Event == SW_LONG_CLICK) 
  {
    ...
    //Random Punctuation Symbol
    char punc = random('!', '/'+1);

    //Random digit
    char num = random('0', '9' + 1);

    //Random Position in Array
    int ranLen = random(len);

    int ranPreventOverwrite = random(len);
    ...
    //Serial.print(arr);
    arr[ranLen] = punc;
    arr[ranPreventOverwrite] = num;

    while(ranLen == ranPreventOverwrite)
    {
      //Randomising ranLen to prevent same random value overwriting positon in array
      ranLen = random(len);
    }
    Serial.print(arr);
